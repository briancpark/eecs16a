...
def gauss_elim(matrix):
    ...
    location_matrix = np.zeros(
        [numrow, numcol]
    )  # matrix of all zeros, and a one at the current location

    while current_loc[0] <= numrow and current_loc[1] <= numcol:
        current_col_bottom = matrix[
            current_loc[0] - 1 :, current_loc[1] - 1
        ]  # current_col_bottom excludes the portion of the current column above current row
