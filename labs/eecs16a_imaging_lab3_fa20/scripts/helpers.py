...
def plotVec2D(vec, label, color="blue"):
    ...
    q = plt.quiver(
        [0], [0], [X], [Y], angles="xy", scale_units="xy", scale=1, color=color
    )
    plt.quiverkey(
        q,
        X,
...
def Gram_Schmidt_rec(orthonormal_vectors, new_vector):
    if len(orthonormal_vectors) == 0:
        ...
        if len(orthonormal_vectors.shape) == 1:
            ortho_vector = new_vector - orthonormal_vectors.dot(
                orthonormal_vectors.T.dot(new_vector)
            )
        else:
            ortho_vector = new_vector - orthonormal_vectors.dot(
                cheap_o_least_squares(orthonormal_vectors, new_vector)
            )
...
def imageResize(imagePath, saveName, newHeight, newWidth):
    ...
    originalIm = Image.open(imagePath).convert(
        mode="L"
    )  # Opening uploaded image as grayscale
    ...
    newIm.save(
        saveName
    )  # Saving image in CWD with name saveName, for eg., 'example.png'
