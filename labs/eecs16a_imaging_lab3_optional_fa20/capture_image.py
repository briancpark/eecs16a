...
# Whether scan delay statements get printed
PRINT_DELAY_STATEMENT = True
...
class Mask(QtWidgets.QWidget):
    ...
    def updateProjector(self, count):
        ...
        QI = QtGui.QImage(
            mask, mask.shape[1], mask.shape[0], QtGui.QImage.Format_Indexed8
        )
        ...
        self.label.setPixmap(
            QtGui.QPixmap.fromImage(QI).scaled(
                int(scaledHeight * scaledWidthFactor), int(scaledHeight)
            )
        )
    ...
    def initUI(self):
        ...
        if self.currCapture == 0:
            ...
            print(
                "Currently displaying on screen %d"
                % (QtWidgets.QDesktopWidget().screenNumber() + 1)
            )

            for s in range(QtWidgets.QDesktopWidget().screenCount()):
                print(
                    "%d) %d x %d"
                    % (
                        s + 1,
                        QtWidgets.QDesktopWidget().screenGeometry(s).width(),
                        QtWidgets.QDesktopWidget().screenGeometry(s).height(),
                    )
                )
    ...
    def updateData(self):
        ...
        if self.count < self.numMeasurements + shiftCorrection:
            ...
            if self.count < shiftCorrection:
                ...
                if (
                    PRINT_DELAY_STATEMENT
                    and self.count > shiftCorrection
                    and self.count < shiftCorrection + 5
                ):
                    print("Time delta between captures in s: %s" % timeDelta)
                elif self.count > shiftCorrection:
                    if (
                        PRINT_DELAY_STATEMENT
                        and timeDelta > self.sleepTime * 1.35
                        or timeDelta < self.sleepTime * 0.65
                    ):
                        print("Time delta between captures in s: %s" % timeDelta)
                ...
                if (
                    self.dataLoc % 100 == 0
                    or self.dataLoc < 10
                    or self.dataLoc > (self.numMeasurements + shiftCorrection - 10)
                    or data == -1
                ):
            ...
            if (self.dataLoc) < self.numMeasurements - 1:
                print(
                    "Lost %s data! :( Please increase timeout!"
                    % ((self.numMeasurements - 1) - self.dataLoc)
                )
            ...
            self.sensor_readings = np.append(
                self.sensor_readings[: self.dataLoc],
                np.reshape(extra_data, (len(extra_data), 1)),
                axis=0,
            )
            ...
            paddedSensorReadings = np.append(
                self.sensor_readings,
                np.transpose([[np.amax(self.sensor_readings)] * padAmount]),
                axis=0,
            )

            if reshapeHeight != int(len(self.sensor_readings) / self.imgWidth):
                print(
                    "Incomplete measurement row. Is your mask matrix size correct? Padding outputs %s times for display."
                    % padAmount
                )
  ...
  """
    ...
    def rescale(self):
        if self.fullscreen:
            ...
            self.dispHeight = (
                QtGui.QDesktopWidget().screenGeometry(self.screen).height()
            )

        self.label.setGeometry(QtCore.QRect(0, 0, self.dispWidth, self.dispHeight))
        QI = QtGui.QImage(
            self.mask.data, self.imgWidth, self.imgHeight, QtGui.QImage.Format_Indexed8
        )
        self.label.setPixmap(
            QtGui.QPixmap.fromImage(QI).scaled(
                int(self.dispWidth), int(self.dispHeight), QtCore.Qt.KeepAspectRatio
            )
        )
    ...
    def go(self):
        ...
        # self.connect(self.timer, QtCore.SIGNAL('timeout()'), self.updateData) # PyQt4 Solution
        self.timer.timeout.connect(self.updateData)  # PyQt5 Solution
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="This program projects a sampling pattern and records the corresponding phototransistor voltage."
    )
    parser.add_argument(
        "-f", "--fps", type=int, default=40, help="frames per second (default = 40)"
    )
    parser.add_argument(
        "--width",
        type=int,
        default=40,
        help="width of the image in pixels (default = 40px)",
    )
    parser.add_argument(
        "--height",
        type=int,
        default=30,
        help="height of the image in pixels (default = 30px)",
    )
    parser.add_argument(
        "--mask",
        default="masks/diag_mask.npy",
        help='saved sampling pattern mask (default = "masks/diag_mask.npy")',
    )
    parser.add_argument(
        "--out",
        default="images/sensor_readings",
        help='output path (default="images/sensor_readings", saved as "images/sensor_readings_0.npy")',
    )
    parser.add_argument(
        "--numCaptures",
        type=int,
        default=1,
        help="number of image captures (for averaging)",
    )
    parser.add_argument(
        "--sleepTime",
        type=int,
        default=120,
        help="sleep time in milliseconds -- time between projector update and data capture",
    )
    parser.add_argument(
        "--brightness",
        type=int,
        default=100,
        help="% of full projector brightness to use",
    )
    parser.add_argument(
        "--timeout", type=int, default=150, help="serial timeout in ms (default = 40)"
    )
