...
def cross_correlation(stationary_signal, sliding_signal):
    ...
    return entire_corr_vec[
        len(sliding_signal) - 1 : len(sliding_signal) - 1 + len(sliding_signal)
    ]
    # old implementation
...
def cross_corr_demo_1():
    ...
    for i in range(6):
        ...
        plt.title(
            "Computed cross-correlation(%d)=%d\n%s\n%s"
            % (i, np.dot(signal1, signal2), str(signal2), str(signal1))
        )
...
def test_correlation_plot(signal1, signal2, lib_result, your_result):
    ...
    ax.plot(
        [x - len(signal2) + 1 for x in range(len(lib_result))],
        lib_result,
        "k",
        label=str_corr,
        lw=1.5,
    ...
    ax.plot(
        [x - len(signal2) + 1 for x in range(len(your_result))],
        your_result,
        "--r",
        label=str_your,
        lw=3,
...
def test_correlation(cross_correlation, signal_one, signal_two):
    ...
    result_lib = np.array(
        [
            np.correlate(signal_one, np.roll(signal_two, k))
            for k in range(len(signal_two))
        ]
    )
...
def test(cross_correlation, identify_peak, test_num):
    ...
    if test_num == 1:
        ...
        test = list_eq(
            cross_correlation(signal_one, signal_two),
            np.convolve(signal_one, signal_two[::-1]),
        )
        if not test:
            print("Test {0} {1} Failed".format(test_num, test_cases[test_num]))
        else:
            print("Test {0} {1} Passed".format(test_num, test_cases[test_num]))
    ...
    if test_num == 2:
        ...
        if not (test1 and test2):
            print(
                "Test {0} {1} Failed: Your peaks [{2},{3}], Correct peaks [3,4]".format(
                    test_num, test_cases[test_num], your_result1, your_result2
                )
            )
        else:
            print(
                "Test {0} {1} Passed: Your peaks [{2},{3}], Correct peaks [3,4]".format(
                    test_num, test_cases[test_num], your_result1, your_result2
                )
            )
    # 3. Virtual Signal
    if test_num == 3:
        transmitted = (
            np.roll(beacon[0], -10)
            + np.roll(beacon[1], -103)
            + np.roll(beacon[2], -336)
        )
        offsets = [0, 0, 0]  # arrival_time(beacon[0:3], transmitted)
        test = (offsets[0] - offsets[1]) == (103 - 10) and (
            offsets[0] - offsets[2]
        ) == (336 - 10)
        ...
        if not test:
            print(
                "Test {0} {1} Failed: Your offsets [{2},{3}], Correct offsets [93,326]".format(
                    test_num, test_cases[test_num], your_result1, your_result2
                )
            )
        else:
            print(
                "Test {0} {1} Passed: Your offsets [{2},{3}], Correct offsets [93,326]".format(
                    test_num, test_cases[test_num], your_result1, your_result2
                )
            )
...
delay_samples2 = 0
received = (
    np.roll(sent_0, delay_samples0)
    + np.roll(sent_1, delay_samples1)
    + np.roll(sent_2, delay_samples2)
)
...
def pltBeacons(delay_samples0, delay_samples1, delay_samples2):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(received_new), plt.title("Received Signal (sum of beacons)"), plt.xlabel(
        "Samples"
...
def sliderPlots():
    interact(
        pltBeacons,
        delay_samples0=(-500, 500, 10),
        delay_samples1=(-500, 500, 10),
        delay_samples2=(-500, 500, 10),
    )
        ...
        for i in range(1, Ncycle):
            c = np.hstack(
                (
                    c,
                    cross_correlation(
                        raw_signal[i * Lperiod : (i + 1) * Lperiod], b.binarySignal
                    ),
                )
            )
        if ib == 0:
...
def plot_average_multiple_signals(beacon_num=0):
    ...
    axarr[1].axvline(
        x=period_ticks[0], color="red", linestyle="--", label="period start"
    )
    axarr[1].axvline(x=period_ticks[1], color="red", linestyle="--")
    axarr[1].set_title("Averaged & Centered Periodic Output for Beacon 1")
    plt.legend()

    stacked_cs = np.abs(cs[beacon_num])[
        : (len(cs[beacon_num]) // period_len) * period_len
    ].reshape(-1, period_len)
    print(
        "Samples Offset of Each Period in Non-Averaged Signal:",
        [np.argmax(s) for s in stacked_cs],
    )
...
def correlation_plots(offset):
    ...
    ax1.set_xlim(-10, 10)
    ax1.plot(
        stationary_coord, stationary_sig, label="infinite periodic stationary signal"
    )
    ax1.plot(
        sliding_coord + offset, sliding_sig, color="orange", label="sliding signal"
    )
    ...
    ax1.set_title(
        "Infinite Periodic Linear Cross Correlation\nCorr Val at offset "
        + str(offset)
        + " is "
        + str(corr[index + 2])
    )
...
def hyperbola_demo_1():
    ...
    for i in range(3):
        hyp = labDemo.draw_hyperbola(
            labDemo.beaconsLocation[i + 1], labDemo.beaconsLocation[0], dist[i + 1]
        )  # Draw hyperbola
        plt.plot(
            hyp[:, 0],
            hyp[:, 1],
            color=colors[i + 1],
            label="Hyperbola for beacon " + str(i + 1),
            linestyle="--",
        )
...
def plot_speakers_demo():
    ...
    isac = 1
    # index of the beacon to be sacrificed
    speakers = labDemo.beaconsLocation
    helper = lambda i: float(speakers[i][0] ** 2 + speakers[i][1] ** 2) / (
        v * TDOA[i]
    ) - float(speakers[isac][0] ** 2 + speakers[isac][1] ** 2) / (v * TDOA[isac])
    helperx = lambda i: float(speakers[i][0] * 2) / (v * TDOA[i]) - float(
        speakers[isac][0] * 2
    ) / (v * TDOA[isac])
    helpery = lambda i: float(speakers[i][1] * 2) / (v * TDOA[i]) - float(
        speakers[isac][1] * 2
    ) / (v * TDOA[isac])
    ...
    if isac != 1:
        y1 = [
            ((helper(1) - helper(isac)) - v * (TDOA[1] - TDOA[isac]) - helperx(1) * xi)
            / helpery(1)
            for xi in x
        ]
    if isac != 2:
        y2 = [
            ((helper(2) - helper(isac)) - v * (TDOA[2] - TDOA[isac]) - helperx(2) * xi)
            / helpery(2)
            for xi in x
        ]
    if isac != 3:
        y3 = [
            ((helper(3) - helper(isac)) - v * (TDOA[3] - TDOA[isac]) - helperx(3) * xi)
            / helpery(3)
            for xi in x
        ]
    ...
    if isac != 1:
        plt.plot(x, y1, label="Equation for beacon 1", color="g")
    if isac != 2:
        plt.plot(x, y2, label="Equation for beacon 2", color="c")
    if isac != 3:
        plt.plot(x, y3, label="Equation for beacon 3", color="y")
    plt.legend()
    labDemo.plot_speakers(plt, labDemo.beaconsLocation[:4], distances)
...
def test_loc(least_squares, construct_system, x_pos, y_pos, inten, debug=False):
    ...
    _, avgs = labDemo.post_processing(raw_signal)
    ...
    if debug:
        ...
        for i in range(len(b)):
            print(
                "Row %d: %.f should equal %.f"
                % (i, A[i][0] * x_pos + A[i][1] * y_pos, b[i])
            )
