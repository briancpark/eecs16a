...
def get_portlist():
    ...
    if len(ports) > 1:
        ...
        print(
            "No serial ports other than COM1 detected. Make sure MSP430 is connected and try again."
        )
    return result
...
def main():
    ...
    while index > len(portlist):
        print(
            "You have only %i COM ports to choose from, please enter an index in [1, %i]"
    ...
    # connect to the port
    subprocess.call(
        ["python", "virtual_oscope.py", "-D", portlist[index - 1], "--debug"]
    )
