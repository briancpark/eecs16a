...
class APS:
    ...
    def __init__(self, file_name, microphoneLocation=[25, 30], testing="", ms=False):
        ...
        for i in range(self.beaconNum):
            self.beaconList.append(
                self.Beacons(
                    self.beaconsLocation[i],
                    self.data["beacon%d" % i][0],
                    "Beacon%d" % i,
                )
            )
    ...
    def demodulate_signal(self, signal):
        ...
        demod_real_base = [
            cos(2 * pi * self.carrierFreq * i / self.samplingRate)
            for i in range(1, len(signal) + 1)
        ]
        demod_imaginary_base = [
            sin(2 * pi * self.carrierFreq * i / self.samplingRate)
            for i in range(1, len(signal) + 1)
        ]
        # Multiply the bases to the signal received
        demod_real = [demod_real_base[i] * signal[i] for i in range(len(signal))]
        demod_imaginary = [
            demod_imaginary_base[i] * signal[i] for i in range(len(signal))
        ]
        # Filter the signals
    ...
    def generate_raw_signal(self, microphoneLocation, noise=False):
        ...
        if noise:
            #             print("Noise!!!!")
            self.rawSignal = np.roll(
                np.tile(self.rawSignal, 10), 2000
            ) + np.random.normal(0, 1, len(self.rawSignal) * 10)
        else:
            #             print("No Noise!!!!")
    ...
    def identify_offsets(self, averaged):
        ...
        shifted = [
            np.roll(avg, len(averaged[0]) // 2 - self.identify_peak(averaged[0]))
            for avg in averaged
        ]
        ...
        # YOUR CODE HERE
        self.offsetsPost = np.array(
            [self.identify_peak(i) - beacon0_offset for i in shifted]
        )
                ...
                c = np.hstack(
                    [
                        ...
                        self.cross_correlation(
                            demod[i * Lperiod : (i + 1) * Lperiod], b.binarySignal
                        ),
                    ]
                )
            cs.append(c)
    ...
    def construct_system(self, speakers, TDOA, v_s, isac=1, plot=0):
        ...
        for i in range(1, len(TDOA)):
            if (
                i != isac
            ):  # if i is not the index of the beacon to be sacrificed, add elements to A and b
                # YOUR CODE HERE
    ...
    def generate_microphone_loc(self):
        ...
        def plot_table_beacons():
            ...
            for i, j in enumerate(self.beaconsLocation):
                plt.text(
                    j[0] - 5 * factor,
                    j[1] + 1 * factor,
                    self.beaconList[i].name,
                    color=colors[i],
                    fontsize=12,
                )
                plt.scatter(j[0], j[1], marker="x", color=colors[i])
            plt.plot(
                25.280241935483872 * factor,
                -17.958603896103895 * factor,
                "o",
                color="black",
                markersize=5,
                label="Table",
        ...
        def onclick(event):
            ...
            plt.text(
                LOC[0] - 4 * factor, LOC[1] + 1 * factor, "Microphone", color="red"
            )
        ...
        plt.text(
            LOC[0] - 15 * factor,
            LOC[1] + 2 * factor,
            "Please place microphone within the table",
            color="red",
        )
    ...
    def plot_speakers(
        self, plt, coords, distances, xlim=None, ylim=None, circle=True, name=False
    ):
        ...
        if circle == True:
            for i, point in enumerate(coords):
                fig.gca().add_artist(
                    plt.Circle(point, distances[i], facecolor="none", ec=colors[i])
                )
    ...
    def simulation_testing(
        ...
        if not least_squares and not construct_system:
            x, y = self.calculate_position(
                self.least_squares, self.construct_system, simulation, TDOA, isac
            )
        else:
            x, y = self.calculate_position(
                least_squares, construct_system, simulation, TDOA, isac
            )
        ...
        if (
            self.ms
            and self.microphoneLocation[0] != 0.25
            and self.microphoneLocation[1] != 0.30
        ):
            print(
                "Original Microphone Position: %0.4f, %0.4f"
                % (self.microphoneLocation[0], self.microphoneLocation[1])
            )
        elif (
            not self.ms
            and self.microphoneLocation[0] != 25
            and self.microphoneLocation[0] != 30
        ):
            print(
                "Original Microphone Position: %0.4f, %0.4f"
                % (self.microphoneLocation[0], self.microphoneLocation[1])
            ...
            print(
                "Default Microphone Position (Not Original): %0.4f, %0.4f"
                % (self.microphoneLocation[0], self.microphoneLocation[1])
            )
        ...
        if not least_squares and not construct_system:
            A, b = self.construct_system(
                simulation, TDOA, self.V_AIR, isac
            )  # for debugging
        else:
            A, b = construct_system(simulation, TDOA, self.V_AIR, isac)  # for debugging
        colors = ["orange", "g", "c", "y", "m", "b", "k"]
        x2 = np.linspace(xmin - xrange * 0.2, xmax + xrange * 0.2, 1000)
        j = 0
        ...
        for i in range(len(b)):
            ...
            plt.plot(
                ...
                y2,
                color=colors[j],
                label="Linear Equation " + str(j),
                linestyle="-",
            )
            plt.xlim(xmin - xrange * 0.2, xmax + xrange * 0.2)
            plt.ylim(ymin - yrange * 0.2, ymax + yrange * 0.2)
        #             plt.legend(bbox_to_anchor=(1.4, 1))
        #             plt.legend()

        for i in range(5):
            hyp = self.draw_hyperbola(
                simulation[i + 1], simulation[0], self.distancesPost[i + 1]
            )  # Draw hyperbola
            plt.plot(
                hyp[:, 0],
                hyp[:, 1],
                color=colors[i + 1],
                label="Hyperbolic Equation " + str(i + 1),
                linestyle=":",
            )
    ...
    class Beacons:
        ...
        def distance(self, microphoneLocation):
            return np.sqrt(
                (self.coordinate[0] - microphoneLocation[0]) ** 2
        ...
        def modulate_signal(self):
            ...
            self.modulatedSignal = [
                self.binarySignal[i] * self.carrierSingal[i]
                for i in range(self.signalLength)
            ]
        ...
        def generate_carrier(self, signalLength, RANDOM_OFFSET=False):
            if RANDOM_OFFSET:
                rand = random()
                carrier_sample = (
                    2 * pi * (self.carrierFreq * sample / self.sampling_freq + rand)
                    for sample in range(1, signalLength + 1)
                )
            else:
                carrier_sample = (
                    2 * pi * self.carrierFreq * sample / self.sampling_freq
                    for sample in range(1, signalLength + 1)
                )
