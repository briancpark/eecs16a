...
class Mask(QtWidgets.QWidget):
    def __init__(
        self, imgWidth, imgHeight, infile, imagefile, sleepTime, noise, noiseFile
    ):
        ...
        self.image = np.array(
            Image.open(imagefile).convert(mode="L")
    ...
    def updateData(self):
        if self.count >= self.numMeasurements:
            ...
            print("\nScan completed")
            print(
                "Scan time: %.3f m" % ((elapsed_time // 60)),
            ...
            img_vec = np.reshape(
                self.image, (self.imgWidth * self.imgHeight, 1)
            )  # Image vector
            ...
            self.label.setPixmap(
                QtGui.QPixmap.fromImage(QI).scaled(self.dispWidth, self.dispHeight)
            )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="width of the image in pixels (default = 32px)",
    )
    parser.add_argument(
        "--height",
        type=int,
        default=32,
        help="height of the image in pixels (default = 32px)",
    )
    parser.add_argument(
        "--mask",
        default="../saved_data/H.npy",
        help='saved sampling pattern mask (default = "../saved_data/H.npy")',
    )
    parser.add_argument(
        "--image",
        default="../saved_data/Home.png",
        help='saved image (default = "../saved_data/Home.png")',
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="noise parameter to simulate non-ideal imaging",
    )
    parser.add_argument(
        "--noiseFile",
        default="",
        help="file containing random noise added to sensor vector",
    )
